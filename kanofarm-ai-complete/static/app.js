...
function chatBubbles(history) {
  return history.map(m => `<div class="msg ${m.role}">${m.role === "assistant" && m.error ? `<span class="err">${esc(m.content)}</span>` : esc(m.content).replace(/\n/g, "<br>")}</div>`).join("")
...
async function vAssistant() {
  ...
  view(card("Ask KanoFarm", `<label for="af">Which farm is this about? (optional)</label><select id="af"><option value="">—</option>${farms}</select>`)
    ...
    + card("", `<textarea id="aq" maxlength="1000" placeholder="e.g. My maize leaves are turning yellow." rows="2"></textarea>
      <button id="ago">Send</button><button id="anew" class="alt">New conversation</button>
      <p class="note">Answers are AI-generated and can be wrong. They do not replace an extension officer. Your LGA and ward, crop and weather advisories are sent to the AI; your name, phone and exact location are not.</p>`));
  CHAT_FARM = "";
  const renderChat = () => { $("#chat").innerHTML = chatBubbles(chatHistory()); $("#chat").scrollTop = $("#chat").scrollHeight; };
  renderChat();
  $("#af").onchange = () => { CHAT_FARM = $("#af").value; renderChat(); };
  $("#anew").onclick = () => { saveChatHistory([]); renderChat(); };
  const send = async () => {
