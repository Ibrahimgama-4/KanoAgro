...
| AI assistant | Optional, multi-turn chat. Off unless `ANTHROPIC_API_KEY` is set (paid). Distinguishes auth, rate-limit, overloaded, timeout and network errors instead of one generic failure |
...
Tested here: 88 unit tests (pure logic). Endpoint tests (`tests_api/`) and app startup run in GitHub Actions because
