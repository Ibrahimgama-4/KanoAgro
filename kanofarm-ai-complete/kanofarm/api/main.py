...
def assistant(payload: dict = Body(...), auth=Depends(current)):
    ...
    req = asst.build_request(config.ASSISTANT_MODEL(), ctx, a["history"], a["message"])
    reply = asst.ask(key, req)   # raises AssistantError, handled by the exception handler above
