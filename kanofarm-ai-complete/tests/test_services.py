...
from kanofarm.services.assistant import build_request, build_context, ask, SYSTEM
...
class AlertsAndAssistant(unittest.TestCase):
    ...
    def test_assistant_request_has_rules_and_no_pii(self):
        ...
        self.assertIn("Never recommend a specific pesticide", req["system"]); self.assertEqual(req["messages"][0]["content"], "why yellow?")
    def test_ask_parses_and_rejects_empty(self):
        self.assertEqual(ask("k", {}, post=lambda k, p: {"content": [{"type": "text", "text": " hi "}]}), "hi")
        with self.assertRaises(ValueError): ask("k", {}, post=lambda k, p: {"content": []})
